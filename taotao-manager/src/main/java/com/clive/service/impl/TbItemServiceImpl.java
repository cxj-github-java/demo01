...
public class TbItemServiceImpl implements TbItemService {
	...
	*/
	@Override
	public TaotaoResult updateItem(List<TbItem> items, Integer type) {
		List<Long> ids = new ArrayList<Long>();
		for (TbItem tbItem : items) {
			ids.add(tbItem.getId());
			
		}
		int count = tbItemMapper.updateItemById(ids, type);
		if(count>0&&type==0){
	...
	@Override
	public LayuiTableResult searchTbItem(String title, Long price, String sellPoint, Integer page, Integer limit) {
		LayuiTableResult result = new LayuiTableResult();
		List<TbItem> data = tbItemMapper.searchTbItem(title,price,sellPoint,(page-1)*limit, limit);
		Integer i  = tbItemMapper.searchCount();
		result.setCode(0);
		result.setCount(i);
		result.setData(data);
		result.setMsg("");
